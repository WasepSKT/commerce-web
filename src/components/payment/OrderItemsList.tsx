          ...
          {items.map((item) => (
            <div key={item.id} className="grid grid-cols-12 gap-2 text-sm items-center">
              <div className="col-span-5 font-medium text-gray-900 flex items-center gap-3">
                ...
                <span>{item.products?.name || `Product ${item.product_id.slice(0, 8)}`}</span>
              </div>
      ...
      <div className="md:hidden space-y-3">
        {items.map((item) => (
          <div key={item.id} className="flex items-start gap-3">
            {item.products?.image_url ? (
              <img src={imageUrlWithCacheBust(item.products.image_url, '')} alt={item.products?.name} className="w-16 h-16 object-cover rounded" />
            ) : (
              <div className="w-16 h-16 bg-gray-200 rounded" />
            )}
            <div className="flex-1 text-sm">
              <div className="font-medium text-gray-900">{item.products?.name || `Product ${item.product_id.slice(0, 8)}`}</div>
              <div className="text-gray-700 text-xs mt-1">Qty: {item.quantity} • Rp {item.price.toLocaleString('id-ID')}</div>
            </div>
            <div className="text-sm font-semibold text-gray-900">Rp {(item.price * item.quantity).toLocaleString('id-ID')}</div>
          </div>
        ))}
      </div>
    </div>
