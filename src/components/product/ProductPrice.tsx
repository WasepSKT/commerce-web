...
export function ProductPrice({ price, discountPercent, formatPrice }: Props) {
  ...
  return (
    <div className="mb-4">
      {hasDiscount ? (
        <div>
          <div className="text-lg text-muted-foreground line-through font-medium">{formatPrice(price)}</div>
          <div className="text-4xl font-bold text-primary">{formatPrice(discountedPrice)}</div>
          <div className="text-sm text-red-600 font-semibold">Diskon {discountPercent}%</div>
        </div>
      ) : (
